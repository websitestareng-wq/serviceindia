...
function openAttachmentAction(
  ...
  openPdfActionModal({
    ...
    const partyName =
      partyOptions.find((party) => party.id === filters.partyId)?.name ||
      "party";
    ...
    openPdfActionModal({
      ...
      fileName: buildSafePdfFileName(
        `${partyName} ${getPageTitle(type)} ${filters.dateFrom} to ${filters.dateTo}.pdf`,
      ),
    });
  }
                    ...
                    <div className="space-y-3.5">
                      <div>
                        ...
                        <select
                          ...
                          className="h-10.5 w-full rounded-[18px] border border-slate-200 bg-white px-4 text-sm text-slate-700 outline-none transition-all duration-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
                        >
                          {partyOptions.map((party) => (
                            <option key={party.id} value={party.id}>
                              {party.name}
        ...
        <td className="px-4 py-3 text-right align-top">
          ...
          {row.settlementRows.length > 0 ? (
            <button
              ...
              className="mt-1 block w-full text-right text-[11px] font-semibold text-blue-700 underline-offset-4 transition hover:text-red-600 hover:underline"
            >
              View Settlement
            </button>
          ) : null}
